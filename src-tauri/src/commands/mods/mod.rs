...
// Installs an already-downloaded Nexus archive (the nxm:// handoff downloads before
// calling this). Nexus mods carry no modworkshop identity, so the entry gets a
// negative id, a nexus-prefixed uid, and source "nexus"; a later index refresh can
// still upgrade it by SHA256 when the same mod is cross-posted on modworkshop.
// The multi-choice picker sentinels fail honestly: their modals are modworkshop-shaped
// and can't be offered for this flow yet.
