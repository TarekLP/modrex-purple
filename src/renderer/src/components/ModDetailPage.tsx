    ...
    return (
        <ReactMarkdown
            components={{
                ...
                ul: ({ children }) => <ul className="list-disc ml-5 mb-2 text-sm text-text-muted">{children}</ul>,
                ol: ({ children }) => <ol className="list-decimal ml-5 mb-2 text-sm text-text-muted">{children}</ol>,
                li: ({ children }) => <li className="mb-0.5">{children}</li>,
                a: ({ href, children }) => (
                    <a
                        onClick={(e) => { e.preventDefault(); href && window.api.openExternal(href) }}
                ...
                code: ({ children }) => (
                    <code className="font-mono text-[0.85em] bg-surface-hover px-1 py-0.5 rounded">{children}</code>
                ),
                img: ({ src, alt }) => (
