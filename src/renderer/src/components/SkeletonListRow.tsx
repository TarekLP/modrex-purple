export function SkeletonListRow() {
    return (
        <div className="flex items-stretch rounded-lg border border-border bg-surface-raised overflow-hidden animate-pulse">
            <div className="shrink-0 w-28 bg-surface-hover" />
            <div className="flex-1 min-w-0 px-5 py-4 flex flex-col gap-2">
                <div className="h-3 bg-surface-hover rounded w-2/3" />
                <div className="h-2.5 bg-surface-hover rounded w-1/3" />
                <div className="h-2.5 bg-surface-hover rounded w-1/4" />
            </div>
            <div className="flex items-center gap-2 px-4 shrink-0">
                <div className="w-9 h-5 bg-surface-hover rounded-full" />
                <div className="w-8 h-8 bg-surface-hover rounded" />
            </div>
        </div>
    )
}
