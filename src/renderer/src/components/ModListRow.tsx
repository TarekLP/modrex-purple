...
export function ModListRow({
    ...
    return (
        <div
            className={`flex items-stretch rounded-lg bg-surface-raised border border-border overflow-hidden transition-opacity ${loading ? 'opacity-60' : ''}`}
        >
            <div onClick={onOpen} className="shrink-0 w-36 bg-surface-hover cursor-pointer">
                {mod.thumbnail ? (
                    <img
                        ...
                        alt=""
                        className={`w-full h-full object-cover transition-[filter] ${!installed.enabled ? 'grayscale' : ''}`}
                    />
                ) : (
            ...
            <button onClick={onOpen} className="flex-1 min-w-0 text-left px-5 py-4">
                <p className="text-base font-bold leading-snug truncate">{mod.name}</p>
