...
- `loaders.rs` (inline `#[cfg(test)] mod tests`) — registry invariants (unique loader ids, every id resolves, every package binding names a registered game and a real loader, a mod id naming at most one loader within a game, loader configuration riding only on the loader it configures) plus per-loader presence detection driven through `check_loader`: SuperBLT's three filenames, RAID-SuperBLT's two hooks and absent Linux variant, PDTHModOverrides requiring the `DINPUT8.dll` proxy, DAHM's hook, the directory-named-like-the-loader edge case, the missing-game-path case, and that raw detection ignores the Diesel 3.0 marker (that carve-out lives in `check_loader`, not in detection)
