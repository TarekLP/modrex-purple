...
pub fn resolve_archive_download(downloaded: PathBuf, cfg: &ModEngineConfig) -> ResolvedArchive {
    ...
    if cfg.game_id == "itr2" {
        return resolve_intotheradius_archive(downloaded, cfg);
    }
...
/// One resolved piece of an Into The Radius 2 archive. ITR2 mods of different formats are
/// indistinguishable by extension (all four are folders of loose files in a zip), so routing
/// is content-signature based, mirroring the Vortex extension's installContent table:
///
///   - a dir holding a .uplugin is an SML mod (its .pak/.ucas/.utoc travel with it)
///   - a dir holding enabled.txt or Scripts/main.lua is a UE4SS Lua mod
///   - a .pak nested under a LogicMods/ segment is a logic mod, named after the dir above it
///   - any other .pak is a standard mod, named after its enclosing dir (or its own stem)
///
/// A standard or logic mod's paks are extracted explicitly (with sidecars) so nothing outside
/// the pak's own folder leaks in; an SML or Lua mod's whole archive dir is copied instead.
pub(crate) struct Itr2Group {
    install_name: String,
    tag: &'static str,
    /// Archive directory copied wholesale when non-empty (SML/Lua/folder-standard mods).
    extract_dir: String,
    /// Explicit .pak entries to extract with sidecars (logic mods and root-level paks).
    paks: Vec<String>,
}

fn itr2_parent_dir(name: &str) -> String {
    Path::new(name)
        .parent()
...
fn itr2_dir_name(dir: &str) -> String {
    Path::new(dir)
        .file_name()
        .and_then(|s| s.to_str())
        .unwrap_or("mod")
...
fn itr2_file_stem(name: &str) -> String {
    Path::new(name)
        .file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("mod")
...
/// The mod folder and paks dir for a pak sitting under a LogicMods/ segment, or None when
/// parent has no such segment. Mirrors the Vortex plugin: the mod folder is the directory
/// above LogicMods (e.g. Mods/LogicMods/WeaponsOverhaul/x.pak installs as WeaponsOverhaul),
/// and only the paks directly under the segment are content, never sibling dirs.
fn itr2_logic_mod(pak_parent: &str) -> Option<(String, String)> {
    if Path::new(pak_parent).file_name()?.to_str()? != "LogicMods" {
        return None;
    }
    let paks_dir = pak_parent.to_string();
    let install_name = Path::new(pak_parent)
        .parent()
        .and_then(|p| p.file_name())
        .and_then(|s| s.to_str())
        .unwrap_or("mod")
        .to_string();
    Some((paks_dir, install_name))
}

pub(crate) fn classify_intotheradius(names: &[String]) -> Vec<Itr2Group> {
    use std::collections::BTreeMap;

    let mut sml_dirs: Vec<String> = Vec::new();
    let mut lua_dirs: Vec<String> = Vec::new();
    for name in names {
        if name.ends_with('/') {
            continue;
        }
        if name.ends_with(".uplugin") {
            let dir = itr2_parent_dir(name);
            if !sml_dirs.contains(&dir) {
                sml_dirs.push(dir);
            }
        }
        if name == "enabled.txt" || name.ends_with("/enabled.txt") {
            let dir = itr2_parent_dir(name);
            if !lua_dirs.contains(&dir) {
                lua_dirs.push(dir);
            }
        }
        // A Lua mod's marker is Scripts/main.lua, but the mod folder is the dir containing
        // the Scripts/ folder (the same dir that carries enabled.txt), not Scripts/ itself.
        if name == "Scripts/main.lua" || name.ends_with("/Scripts/main.lua") {
            let dir = name
                .strip_suffix("/Scripts/main.lua")
                .map(|d| d.to_string())
                .unwrap_or_default();
            if !lua_dirs.contains(&dir) {
                lua_dirs.push(dir);
    ...
    let mut groups: BTreeMap<String, Itr2Group> = BTreeMap::new();
    for dir in &sml_dirs {
        groups.insert(
            format!("sml:{dir}"),
            Itr2Group {
                install_name: if dir.is_empty() {
                    // .uplugin at the archive root with no enclosing folder.
                    "mod".to_string()
                } else {
                    itr2_dir_name(dir)
                },
    ...
    for dir in &lua_dirs {
        groups.insert(
            ...
            Itr2Group {
                install_name: if dir.is_empty() {
                    // enabled.txt or Scripts/main.lua at the archive root with no enclosing folder.
                    "mod".to_string()
                } else if let Some(shared) = dir.strip_prefix("shared/") {
                    // A shared library installs into LuaMods/shared/<name>/, and the archive
                    // usually ships it already under a shared/ segment, so keep that nesting.
    ...
    // A dir holding a .uplugin is an SML mod copied wholesale, so paks under it (its own
    // .pak/.ucas/.utoc) must not also become their own standard group.
    let under_sml = |dir: &str| {
        sml_dirs
            .iter()
            .any(|a| *a == dir || dir.starts_with(&format!("{a}/")))
    };

    for name in names {
        if name.ends_with('/') || !name.ends_with(".pak") {
            continue;
        }
        let parent = itr2_parent_dir(name);
        if under_sml(&parent) {
            continue;
        }
        if let Some((paks_dir, install_name)) = itr2_logic_mod(&parent) {
            let key = format!("logic:{paks_dir}");
            let entry = groups.entry(key).or_insert_with(|| Itr2Group {
                install_name,
                tag: "logicmods",
                extract_dir: String::new(),
                paks: Vec::new(),
            });
            entry.paks.push(name.clone());
            continue;
        }
        let key = format!("paks:{parent}");
        let root = parent.is_empty();
        let entry = groups.entry(key).or_insert_with(|| Itr2Group {
            install_name: if root {
                itr2_file_stem(name)
            } else {
                itr2_dir_name(&parent)
            },
            tag: "paks",
            extract_dir: if root { String::new() } else { parent.clone() },
            paks: Vec::new(),
        });
        if root {
            entry.paks.push(name.clone());
...
/// Into The Radius 2 has four mod formats with no distinguishing file extension, so the
/// archive is classified by content signature (see Itr2Group). Every target is a Directory
/// unit, so the result is always a folder ready to be copied under the routed target's
/// mods_subpath. The UE4SS loader package is detected up front: Lua mods depend on it, and
/// the loader's own bundled Scripts/main.lua sub-mods would otherwise classify as Lua mods.
fn resolve_intotheradius_archive(downloaded: PathBuf, cfg: &ModEngineConfig) -> ResolvedArchive {
    if detect_archive(&downloaded).is_none() {
        // A bare file dropped onto the window (no archive). Wrap it in a folder so the
        // Directory-unit targets can install it, routing by extension: a lone .uplugin can
        // only be an SML mod, a lone .pak can only be a standard pak mod.
        let ext = downloaded
            .extension()
            .and_then(|e| e.to_str())
            .unwrap_or("")
            .to_ascii_lowercase();
        let tag = if ext == "uplugin" { "sml" } else { "paks" };
        let stem = downloaded
            .file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("mod")
            .to_string();
        let tmp_parent = std::env::temp_dir().join(format!("modrex-mod-{}", Uuid::new_v4()));
        let tmp = tmp_parent.join(&stem);
        std::fs::create_dir_all(&tmp).map_err(|e| e.to_string())?;
        let filename = downloaded
            .file_name()
            .and_then(|s| s.to_str())
            .unwrap_or("mod");
        std::fs::copy(&downloaded, tmp.join(filename)).map_err(|e| e.to_string())?;
        let location_tag = (tag != cfg.primary().tag).then(|| tag.to_string());
        return Ok((tmp, None, location_tag));
    }
    let names: Vec<String> = list_entries(&downloaded)?
        .into_iter()
        .map(|e| {
    ...
    let groups = classify_intotheradius(&names);
    if groups.is_empty() {
        let engine_markers: Vec<&str> = cfg
            .targets
            .iter()
        ...
        let _ = std::fs::remove_file(&downloaded);
        return Err(ResolveError::Failure(
            "This mod is packaged as an archive with no known Into The Radius 2 mod content \
             (.uplugin, enabled.txt / Scripts/main.lua, or .pak) found inside."
                .to_string(),
        ));
    }
    if groups.len() == 1 {
        let group = &groups[0];
        let tmp_parent = std::env::temp_dir().join(format!("modrex-mod-{}", Uuid::new_v4()));
        let tmp = tmp_parent.join(&group.install_name);
        std::fs::create_dir_all(&tmp).map_err(|e| e.to_string())?;
        if !group.extract_dir.is_empty() {
            extract_dir_entry(&downloaded, &group.extract_dir, &tmp)?;
        } else if !group.paks.is_empty() {
            for pak in &group.paks {
                let filename = Path::new(pak)
                    .file_name()
                    .and_then(|s| s.to_str())
                    .unwrap_or("mod");
                extract_entry_with_sidecars(&downloaded, pak, &tmp.join(filename))?;
            }
        } else {
            // Markers at the archive root with no enclosing folder (a bare .uplugin or
            // enabled.txt): copy the whole archive, which for this shape is all content.
            extract_archive_flat(&downloaded, &tmp)?;
        }
        let location_tag = (group.tag != cfg.primary().tag).then(|| group.tag.to_string());
        return Ok((tmp, Some(downloaded), location_tag));
    }
    let zip_path = downloaded.to_string_lossy().to_string();
    let entry_names: Vec<String> = groups
        .iter()
        .map(|g| {
            if !g.extract_dir.is_empty() {
                g.extract_dir.clone()
            } else {
                g.paks
                    .first()
                    .map(|p| itr2_parent_dir(p))
                    .unwrap_or_default()
            }
        })
        .collect();
    let tag_for = |t: &str| (t != cfg.primary().tag).then(|| t.to_string());
    let distinct_tags: HashSet<&str> = groups.iter().map(|g| g.tag).collect();
    let payload = multi_pak_payload(
        zip_path,
        entry_names,
        if distinct_tags.len() == 1 {
            tag_for(groups[0].tag)
        } else {
            None
        },
        (distinct_tags.len() > 1).then(|| groups.iter().map(|g| tag_for(g.tag)).collect()),
        Some("dir".to_string()),
    );
    Err(prompt_err(InstallPrompt::ZipMultiPak(payload)))
}
