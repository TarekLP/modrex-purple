...
fn resolve_pak_path_cb_legacy_paks_target() {
    ...
    let resolved =
        resolve_pak_path(game, cfg, &[], &pak_entry("001_Foo.pak", Some("paks"))).unwrap();
    assert_eq!(resolved, pak);
}
...
// ── Into The Radius 2: resolve_archive_download ──────────────────────────────

fn itr2_cfg() -> &'static ModEngineConfig {
    engine_for_game("itr2").unwrap()
}

#[test]
fn itr2_sml_mod_resolves_to_sml_target() {
    // A dir holding a .uplugin is an SML mod: its paks travel with it and the whole dir is
    // copied, so the paks must not re-classify as their own standard group.
    let zip = make_zip(&[
        ("CoolGun/CoolGun.uplugin", b"uplugin"),
        ("CoolGun/CoolGun.pak", b"pak"),
        ("CoolGun/CoolGun.ucas", b"ucas"),
        ("CoolGun/CoolGun.utoc", b"utoc"),
    ]);
    let (extracted, _orig, location_tag) =
        resolve_archive_download(zip.path().to_path_buf(), itr2_cfg()).unwrap();
    assert_eq!(location_tag, None, "sml is the primary target");
    assert_eq!(extracted.file_name().unwrap(), "CoolGun");
    assert!(extracted.join("CoolGun.pak").exists());
    assert!(extracted.join("CoolGun.ucas").exists());
    assert!(extracted.join("CoolGun.utoc").exists());
    assert!(extracted.join("CoolGun.uplugin").exists());
}

#[test]
fn itr2_standard_folder_pak_resolves_to_paks_target() {
    let zip = make_zip(&[
        ("BetterWeapons/BP.pak", b"pak"),
        ("BetterWeapons/BP.ucas", b"ucas"),
    ]);
    let (extracted, _orig, location_tag) =
        resolve_archive_download(zip.path().to_path_buf(), itr2_cfg()).unwrap();
    assert_eq!(location_tag.as_deref(), Some("paks"));
    assert_eq!(extracted.file_name().unwrap(), "BetterWeapons");
    assert!(extracted.join("BP.pak").exists());
    assert!(extracted.join("BP.ucas").exists());
}

#[test]
fn itr2_root_level_pak_resolves_to_paks_target_named_after_stem() {
    let zip = make_zip(&[("WeaponMod_P.pak", b"pak"), ("WeaponMod_P.ucas", b"ucas")]);
    let (extracted, _orig, location_tag) =
        resolve_archive_download(zip.path().to_path_buf(), itr2_cfg()).unwrap();
    assert_eq!(location_tag.as_deref(), Some("paks"));
    assert_eq!(extracted.file_name().unwrap(), "WeaponMod_P");
    assert!(extracted.join("WeaponMod_P.pak").exists());
    assert!(extracted.join("WeaponMod_P.ucas").exists());
}

#[test]
fn itr2_logic_mod_resolves_to_logicmods_target() {
    // The mod folder is the dir above the LogicMods segment, and only the paks under the
    // segment are content — extracted explicitly so nothing else in the wrapper leaks in.
    let zip = make_zip(&[
        ("Mods/WeaponsOverhaul/LogicMods/weapons.pak", b"pak"),
        ("Mods/WeaponsOverhaul/LogicMods/weapons.ucas", b"ucas"),
        ("Mods/WeaponsOverhaul/readme.txt", b"not content"),
    ]);
    let (extracted, _orig, location_tag) =
        resolve_archive_download(zip.path().to_path_buf(), itr2_cfg()).unwrap();
    assert_eq!(location_tag.as_deref(), Some("logicmods"));
    assert_eq!(extracted.file_name().unwrap(), "WeaponsOverhaul");
    assert!(extracted.join("weapons.pak").exists());
    assert!(extracted.join("weapons.ucas").exists());
    assert!(!extracted.join("readme.txt").exists());
}

#[test]
fn itr2_lua_mod_resolves_to_lua_mods_target() {
    let zip = make_zip(&[
        ("CoolMod/enabled.txt", b""),
        ("CoolMod/Scripts/main.lua", b"-- lua mod"),
    ]);
    let (extracted, _orig, location_tag) =
        resolve_archive_download(zip.path().to_path_buf(), itr2_cfg()).unwrap();
    assert_eq!(location_tag.as_deref(), Some("lua_mods"));
    assert_eq!(extracted.file_name().unwrap(), "CoolMod");
    assert!(extracted.join("Scripts").join("main.lua").exists());
    assert!(extracted.join("enabled.txt").exists());
}

#[test]
fn itr2_ue4ss_loader_package_resolves_to_loader_sentinel() {
    // The loader ships UE4SS-settings.ini plus its own bundled Scripts/main.lua sub-mods,
    // which would otherwise classify as Lua mods.
    let zip = make_zip(&[
        ("UE4SS-settings.ini", b"[Loader]"),
        ("Mods/ConsoleCommandsMod/Scripts/main.lua", b"-- bundled"),
    ]);
    let err = resolve_archive_download(zip.path().to_path_buf(), itr2_cfg()).unwrap_err();
    assert!(matches!(err, ResolveError::Ue4ssLoader(_)), "{err:?}");
}

#[test]
fn itr2_bare_pak_drop_wraps_in_folder_and_routes_to_paks() {
    let tmp = TempDir::new().unwrap();
    let path = tmp.path().join("LooseMod.pak");
    fs::write(&path, b"loose pak bytes").unwrap();
    let (extracted, orig, location_tag) =
        resolve_archive_download(path.clone(), itr2_cfg()).unwrap();
    assert_eq!(orig, None);
    assert_eq!(location_tag.as_deref(), Some("paks"));
    assert_eq!(extracted.file_name().unwrap(), "LooseMod");
    assert!(extracted.join("LooseMod.pak").exists());
}

#[test]
fn itr2_bare_uplugin_drop_wraps_in_folder_and_is_primary() {
    let tmp = TempDir::new().unwrap();
    let path = tmp.path().join("CoolGun.uplugin");
    fs::write(&path, b"uplugin").unwrap();
    let (extracted, orig, location_tag) =
        resolve_archive_download(path.clone(), itr2_cfg()).unwrap();
    assert_eq!(orig, None);
    assert_eq!(location_tag, None, "sml is the primary target");
...
#[test]
fn itr2_mixed_archive_prompts_multi_pak_with_dir_entries() {
    let zip = make_zip(&[
        ("CoolGun/CoolGun.uplugin", b"uplugin"),
        ("LuaCool/enabled.txt", b""),
        ("LuaCool/Scripts/main.lua", b"-- lua"),
    ]);
    let err = resolve_archive_download(zip.path().to_path_buf(), itr2_cfg()).unwrap_err();
    let ResolveError::Prompt(prompt) = err else {
        panic!("expected a prompt, got {err:?}");
    };
    let InstallPrompt::ZipMultiPak(payload) = *prompt else {
        panic!("expected a multi-pak prompt");
    };
    assert_eq!(payload.entry_kind.as_deref(), Some("dir"));
    assert_eq!(payload.entries.len(), 2);
    assert!(payload.entries.iter().any(|e| e == "CoolGun"));
    assert!(payload.entries.iter().any(|e| e == "LuaCool"));
    assert_eq!(payload.entry_tags.as_ref().map(|t| t.len()), Some(2));
}

#[test]
fn itr2_classifier_skips_paks_under_sml_dirs() {
    let names: Vec<String> = [
        "CoolGun/CoolGun.uplugin",
        "CoolGun/CoolGun.pak",
        "CoolGun/CoolGun.ucas",
        "Standalone/BP.pak",
    ]
    .iter()
    .map(|s| s.to_string())
    ...
    assert_eq!(
        groups.len(),
        2,
        "the SML dir's paks must not become their own group"
    );
}

#[test]
fn itr2_flat_uninstallable_archive_prompts_unrecognized_and_removes_source() {
    let zip = make_zip(&[("readme.txt", b"nothing installable here")]);
    let zip_path = zip.path().to_path_buf();
    let err = resolve_archive_download(zip_path.clone(), itr2_cfg()).unwrap_err();
    assert!(
        matches!(&err, ResolveError::Prompt(p) if matches!(**p, InstallPrompt::UnrecognizedArchive)),
        "{err:?}"
    );
    assert!(
        !zip_path.exists(),
        "an unrecognized archive must be removed so the user isn't re-prompted on retry"
    );
}
