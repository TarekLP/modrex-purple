...
fn pdth_engine() -> &'static ModEngineConfig {
    engine_for_game("pdth").unwrap()
}

fn cb_engine() -> &'static ModEngineConfig {
    engine_for_game("cb").unwrap()
}

#[test]
fn only_crime_boss_resyncs_enabled_flags() {
    assert!(resyncs_enabled_flags(cb_engine()));
...
fn dropped_filename_uses_the_recovered_stem_for_directories() {
    ...
    assert_eq!(
        install_filename_for_dropped(cb_engine(), target(cb_engine(), "mods"), "CoolMod"),
        "CoolMod"
    );
    assert_eq!(
        install_filename_for_dropped(cb_engine(), target(cb_engine(), "paks"), "CoolMod"),
        "CoolMod.pak"
    );
}
...
fn zip_entry_filename_keeps_the_entry_name_off_crime_boss() {
    ...
    assert_eq!(
        install_filename_for_zip_entry(cb_engine(), "CoolMod", "Inner Entry.pak"),
        "CoolMod"
    );
}
...
fn cb_dir_entry_needs_both_crime_boss_and_a_dir_entry_kind() {
    assert!(is_cb_dir_entry(cb_engine(), Some("dir")));
    assert!(!is_cb_dir_entry(cb_engine(), Some("pak")));
    assert!(!is_cb_dir_entry(cb_engine(), None));
...
fn entry_staging_wraps_only_crime_boss_pak_entries() {
    assert_eq!(
        entry_staging(cb_engine(), target(cb_engine(), "paks"), false),
        EntryStaging::CrimeBossSkeleton
    );

    // A Crime Boss entry classified as a directory is staged like every other directory.
    assert_eq!(
        entry_staging(cb_engine(), target(cb_engine(), "paks"), true),
        EntryStaging::DirectoryUnderNewParent
    );
    assert_eq!(
        entry_staging(cb_engine(), target(cb_engine(), "ue4ss_mods"), true),
        EntryStaging::DirectoryUnderNewParent
    );
...
fn entry_extraction_skips_only_the_already_staged_crime_boss_entry() {
    assert_eq!(
        entry_extraction(cb_engine(), target(cb_engine(), "paks"), false),
        EntryExtraction::AlreadyStaged
    );
    assert_eq!(
        entry_extraction(cb_engine(), target(cb_engine(), "mods"), false),
        EntryExtraction::AlreadyStaged
    );
    assert_eq!(
        entry_extraction(cb_engine(), target(cb_engine(), "paks"), true),
...
fn crime_boss_differs_from_other_games_only_where_production_differs() {
    ...
    let tmp = PathBuf::from("/tmp/modrex-mod-abc/OnDiskName");
    let cb_dir = target(cb_engine(), "mods");
    let raid_dir = target(raid_engine(), "mods");
    let cb_file = target(cb_engine(), "paks");
    ...
    assert_ne!(
        entry_extraction(cb_engine(), cb_file, false),
...
fn filename_decisions_sanitize_through_naming() {
    ...
    assert_eq!(
        install_filename_for_dropped(cb_engine(), target(cb_engine(), "mods"), "Cool Mod"),
        "Cool_Mod"
    );
}
