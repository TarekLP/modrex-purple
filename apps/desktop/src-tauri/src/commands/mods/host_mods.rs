//! Registry of host mods, meaning mods whose add-on content packs install inside another
//! mod's folder rather than at a scan-target root. The canonical case is Menu Backgrounds
//! (modworkshop 17160): a background set is a folder of images belonging in
//! mods/Menu Backgrounds/Assets/, not in mods/ or assets/mod_overrides/. Such packs carry
//! no marker and no asset-DB structure, so target classification cannot route them and they
//! are recognized here by content signature instead. The set of host mods is small and
//! stable (PAYDAY 2 is long frozen), so a hardcoded table fits. The host's on-disk folder is
//! resolved from installed state by host_mod_id, never assumed from the name.

/// A host mod that accepts nested content packs, plus how to recognize a pack for it.
pub struct HostTarget {
    /// modworkshop id of the host mod. Its on-disk folder is resolved from state by this.
    pub host_mod_id: i64,
    ...
    /// the host's own bundled content is not mistaken for a user-installed pack.
...
/// A recognized host-mod content pack: the host it targets and the pack directories found
/// inside the archive (each becomes one set installed under the host's subpath).
...
/// Recognizes whether an archive (given its slash-separated entry names) is a content pack
/// for a known host mod. Returns the matching host and its pack directories, or None.
...
/// Directories whose immediate child files include at least host.min_matches distinct
/// signature stems (with a pack-image extension). Each such directory is one content set.
