...
pnpm build:signed # CI production build with updater artifacts and Authenticode signing — requires TAURI_SIGNING_PRIVATE_KEY and, on Windows, the AZURE_* secrets read by src-tauri/scripts/sign-windows.ps1 (release.yml only; exits 1 without them)
