...
if (!query) {
    console.log('Usage: node lookup-mod.mjs <mod name or remote_id>')
    process.exit(1)
}
...
if (mods.length === 0) {
    ...
    for (const mod of mods) {
        const files = db
            .prepare('SELECT sha256, remote_id, version FROM files WHERE mod_id = ?')
            .all(mod.id)
        console.log(`\n"${mod.name}" (remote_id=${mod.remote_id})`)
        console.log(`  ${files.length} pak hash(es) indexed:`)
        files.forEach((f) =>
            console.log(`    sha256=${f.sha256.slice(0, 16)}… file_id=${f.remote_id} v${f.version}`)
        )
    }
}
