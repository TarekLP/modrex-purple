...
- **`mods/`** — the most complex module, split into focused subfiles. `mod.rs` holds all `#[tauri::command]` functions and re-exports; the logic lives in:
    - `engine.rs` — `ModEngineConfig` static dispatch struct that parameterizes all engine behavior per game. Fields: `game_id`, `index_game_name` (matches `games.name` in `index.db` — "PAYDAY 3", "PAYDAY 2", "PAYDAY: The Heist"), `unit: ModUnit`, `state_filename`, `mods_subpath`, `disabled_subpath`, `backup_subpath`. `ModUnit` enum: `File { extension, disabled_suffix, priority_prefix }` for UE5 pak games (PD3), `Directory { entry_marker, priority_prefix }` for Diesel/BLT games (PD2, PDTH). `engine_for_game(game_id)` dispatches: `"pd2"` → `PD2_ENGINE`, `"pdth"` → `PDTH_ENGINE`, anything else → `PD3_ENGINE`. Helper fns: `mods_dir`, `disabled_dir`, `backup_dir`, `state_path`. Methods: `disabled_suffix()`, `is_directory_unit()` (true for PD2/PDTH, false for PD3). Every Tauri command in `mod.rs` starts with `let cfg = engine_for_game(game_id.as_deref().unwrap_or("pd3"))` and passes `cfg` to all lower-level ops.
    ...
    - `paths.rs` — `mods_base` / `disabled_base` / `get_state_path` / `active_mod_path` / `disabled_mod_path` / `find_untracked_mods` (dispatches on `cfg.unit`: scans `*.pak` / `*.pak.disabled` for File; scans subdirectories containing `entry_marker` for Directory)
    ...
    - `install.rs` — `install_mod_from_path`, `uninstall_mod_op`, `enable_mod_op`, `disable_mod_op`. All accept `cfg: &ModEngineConfig` and dispatch on `cfg.unit`: File ops use extension rename / file copy / remove_file; Directory ops use dir move / recursive copy / remove_dir_all. Priority prefix applied only when `cfg.unit.priority_prefix` is true. `uninstall_mod_op` calls `prune_empty_folders` after removing the mod entry — this walks up the ancestor chain and removes any folder (from state and disk) that becomes empty (no mods, no child folders) after the uninstall.
...
**`App.tsx`** — owns `installed`, `gamePath`, `modsHidden`, `activeGame` as sources of truth. `detailStack` is `{ modId: number; initialMod?: Mod }[]` — each entry carries the optional browse-list `Mod` object so `ModDetailPage` can render the header instantly before the full fetch completes. `openDetail(modId, from, initialMod?)` resets the stack to a single entry; `pushDetail(modId)` appends (or truncates to an existing entry) without an `initialMod`. `View` type includes `'welcome'` in addition to the nav views. `getInitialView()` returns `'welcome'` when `modrex:active-game` is absent from localStorage (first launch). `activeGame` initializer reads localStorage and accepts any valid `GameId` (`pd2`, `pdth`, `pd3`) — never assume it defaults to `pd3`. `handleGameChange(g)` synchronously restores `gamePath` from `gamePathCache` and restores `installed`/`folders`/`modsHidden` from `installedCache` (so the UI never flashes "game not found" or "no mods installed" while re-validation runs in the background), saves the game to localStorage, and navigates to the saved nav view. Falls back to empty state when a game has never been loaded this session. `installedReady` is NOT reset on game switch — it stays `true` to keep the splash screen hidden. `handleShowWelcome()` navigates back to the welcome screen. When `view === 'welcome'` the entire TopBar/Sidebar is hidden — only `WelcomeScreen` renders inside the root div. App version shown in `TopBar.tsx` comes from `import.meta.env.VITE_APP_VERSION`, injected by Vite's `define` in `vite.config.ts` from `package.json` at build time; shows `v-dev` in dev mode. `TopBar` receives `activeGame: GameId` — the `-fileopenlog` launch-option warning is PD3-only and is suppressed for other games; `SettingsPage` likewise hides the `-fileopenlog` description and placeholder for non-PD3 games. Only PD3 requires `-fileopenlog`; PD2 and PDTH do not. On every window focus (500ms debounce) both `refreshGamePath` and `refreshInstalled` run. `BrowsePage` unmounts on tab switch but shows the last result instantly from `browseCache.ts` on remount (SWR: show cached, re-fetch if stale); `InstalledPage` and `SettingsPage` are CSS-`hidden` when inactive to preserve scroll/drag state. `SettingsPage` uses `key={activeGame}` to force a full remount on game change, ensuring stale per-game state (launcher, paths) never bleeds through.
...
- PD2/PDTH mods are BLT directories containing `mod.txt`. Active: `{gamePath}/mods/{modFolder}/`. Disabled: `{gamePath}/mods/disabled/{modFolder}/`. State: `.modrex.json` inside `{gamePath}/mods/`. No numeric priority prefix — UE5-style priority does not apply; BLT loads alphabetically. Disabling moves the whole folder; no extension rename.
- "Launch without mods" for PD3 renames `~mods` → `PAYDAY3/Content/~mods.bak` (one level above `Paks/`) — must be outside `Paks/` because UE5 scans all subdirectories there. For BLT games (PD2/PDTH) the `mods/` directory is kept in place; only user mod subdirectories (everything except `base/`) are moved into `mods.bak/`. `base/` is left untouched because BLT recreates it if missing, which shows a "base mod missing" dialog. `restore_mods` / `do_restore` reverse this: for PD3 renames `~mods.bak` back; for BLT moves entries from `mods.bak/` back into `mods/` then removes `mods.bak/` with `fs::remove_dir` (not `remove_dir_all`) so any entry that failed to rename is never silently deleted.
...
Rust unit tests live in separate test files referenced from the module via `#[cfg(test)] mod tests;`. 72 tests across 5 modules — run with `cargo test` inside `src-tauri/`. `tempfile` and `filetime` crates are in `[dev-dependencies]` for filesystem tests.
