...
**localStorage key convention** — Two scopes, both using the `pd3mm:` prefix's replacement:

- App-level (same across games): `modrex:<key>` — e.g. `modrex:active-view`, `modrex:sidebar-collapsed`, `modrex:mod-cache`
- Game-scoped (per game): `` `modrex:${GAME_STORAGE_KEY}:<key>` `` — e.g. `modrex:pd3:installed-view`, `modrex:pd3:browse-sort`, `modrex:pd3:collapsed-folders`
