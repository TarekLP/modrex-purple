...
- **`settings.ts`** — reads/writes `settings.json` in `app.getPath('userData')`. Fields: `gamePath?` (manual override), `launchOptions?` (extra CLI args for PD3), `skipFileOpenLogWarning?` (suppress the `-fileopenlog` reminder).
