...
- `Mod.download` is `| null` even when `mod.has_download` is true — this happens when a mod has files but no default download set. Always guard with `mod.download &&` before accessing `mod.download.type` or similar fields; `mod.download?.type !== 'pak'` evaluates to `true` when null (since `undefined !== 'pak'`) and causes spurious warnings.
