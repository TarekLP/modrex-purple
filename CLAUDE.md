...
- `components/ManageFilesModal.tsx` — portal modal (`createPortal` into `document.body`, avoiding drag-behind from parent draggable div) for per-file management of multi-pak mods. Groups files by their real `folderId` from raw `installed` (bypasses `normalizeModScopes`). Renders root-level files flat, folder groups as collapsible rows with a chevron + folder name + count + Toggle + delete button. File rows show filename + Toggle + delete. Folder-level and file-level toggle/delete operate independently.
