...
- PD3 mods are `.pak` files. Active: `{gamePath}/PAYDAY3/Content/Paks/~mods/`. Disabled: `~mods/disabled/`. `gamePath` is the Steam library root (`steamapps/common/PAYDAY3`). State is stored as `.pd3mm.json` inside `~mods/` so it travels with the game folder on dual-boot setups.
- "Launch without mods" renames `~mods` → `PAYDAY3/Content/~mods.bak` (one level above `Paks/`) before opening Steam — must be outside `Paks/` because UE5 scans all subdirectories there.
