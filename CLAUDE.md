...
- **`mods/`** — the most complex module, split into focused subfiles. `mod.rs` holds all `#[tauri::command]` functions and re-exports; the logic lives in:
    ...
    - `state.rs` — `read_state`, `save_state`, `get_folder_path` (recursive, builds slash-separated path from `parentId` chain), `reconcile_state` (marks missing files `missing: true`; migrates legacy `.disabled` paths; migrates `.pd3mm.json` → `.modrex.json` on first run; when backup dir exists reads state from there)
