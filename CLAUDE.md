...
`createDragImage` in `hooks/useDragDrop.ts` builds DOM elements outside React's render tree where Tailwind classes don't apply. Use CSS custom properties directly in inline styles (`var(--color-surface-raised)`, `var(--color-border)`, etc.) — they resolve correctly on any DOM element.
