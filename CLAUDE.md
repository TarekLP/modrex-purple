...
Icons: `lucide-react`, imported individually by name. Custom dropdowns: use `components/Select.tsx` — native `<select>` can't be themed. Toggles: use `components/Toggle.tsx`. Markdown: use `components/MarkdownContent.tsx` — never inline `ReactMarkdown` directly. It renders full HTML via `rehype-raw` and GFM tables/strikethrough via `remark-gfm`. Before passing text to ReactMarkdown it pre-splits at two levels: (1) modworkshop `!!! Title ... !!!` collapsible blocks are extracted and rendered as `<details>/<summary>` elements, each body getting its own `ReactMarkdown` instance so markdown (tables, etc.) inside them parses correctly; (2) `![](url)` tokens matching a known video platform are extracted and rendered as `EmbedPlayer` components (thumbnail that loads an iframe on click). Embed platform logic lives in `src/renderer/src/embeds.ts` — adding a new platform means adding one `EmbedDef` entry to `EMBEDS`. `MarkdownContent` accepts an optional `embeds` prop to pass a game-specific subset or superset of platforms. Skeleton loading: use `components/SkeletonCard.tsx` (grid) and `components/SkeletonListRow.tsx` (list) — never replace a grid or list with a centered loading string.
