...
- **`launchers/`** — split into focused files so touching one launcher never requires touching another:
    ...
    - `steam.rs` — Windows: `reg query` for install path, then walks `libraryfolders.vdf`; Linux: checks `STEAM_DIR`, `XDG_DATA_HOME`, snap, flatpak paths. `find_game` requires `game.executable` to exist inside the candidate folder — a leftover folder without the exe is not counted as installed.
    ...
    - `mod.rs` — orchestration (`identify_launcher_for_path`, `launch_with`, `all_launchers`) + all Tauri commands. `game_def_for_id(game_id: &str) -> &'static GameDef` maps `"pd2"` to `&PD2`, anything else to `&PD3` — replaces the old `const GAME` binding. Marker files for `identify_launcher_for_path`: `steam_appid.txt`, `.egstore/`, `MicrosoftGame.config`. `configure_game_path(None)` validates the existing saved path first (checks `game.executable` exists there); only re-detects if the path is stale or absent, then saves. `findGamePath()` in `api.ts` always calls `configure_game_path(null)` on every refresh — this is how stale paths (game uninstalled but folder left behind) get cleared.
...
**`browseCache.ts`** — in-memory SWR cache for the Browse page. Keyed by `(page, query, sort, categoryId)`. 5-minute TTL for mod list pages; 1-hour TTL for categories. `getBrowseCache` returns `{ result, stale }` or `null` — callers show the cached result immediately and skip or background-fetch based on `stale`. Not persisted to localStorage (lives only for the process lifetime). `BrowsePage` reads the initial cache synchronously during component init so the first render already has data on return visits.
