...
- **`components/BrowsePage.tsx`** — paginated mod grid with search, category filter, and sort. Receives `installed`, `onRefreshInstalled`, and `onOpenDetail` as props. Calls `onRefreshInstalled` after any install/uninstall/enable/disable. On Install click: always fetches full mod via `getMod` first. If `mod.download === null` and the mod has more than one file, fetches the file list and shows `FileSelectModal` — user picks the variant. Otherwise checks for missing required deps and shows `DepsWarningModal` if any exist and haven't been dismissed.
